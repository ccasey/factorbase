...
sub factor_basic() {
  ...
  while(<FACTOR_H>) {
    ...
    my $num_length = length($factor);
    &logit("Factor of $num_id found by $method: c$num_length $factor\n");
    $Factors{basic}++;
    $factor_count++;
  }
  close(FACTOR_H);
...
sub factor_mpqs() {
  ...
  while(<FACTOR_H>) {
	...
	my $num_length = length($factor);
        &logit("Factor of $num_id found by MPQS: c$num_length $factor\n");
	$Factors{mpqs}++;
        $factor_count++;
      }
    }
...
sub factor_ecm() {
  ...
  # make things look nice.. i suppose
    $curve_dot++;
    print ".";
    if(($curve_dot == 50) || ($factor) || ($CI)){
     my $curve_bench_end = time();
     my $curve_bench = ($curve_bench_end - $curve_bench_start)/$curve_count;
     printf(" %i @ %0.2f sec/curve\n" ,$curve_count, $curve_bench);
     $curve_dot = 0;
    }
 ...
 &to_disk("REPORT_WORK $num_id $n ECM B1=$b1 CURVES=$curve_count");
  ...
  if($factor) {
    ...
    my $num_length = length($factor);
    &logit("Factor of $num_id found by ECM: c$num_length $factor\n");
  }
}
...
sub do_work() {
  ...
  &logit(">> " . $machine . "\n");
    ...
    if("$method" eq "BASIC") {
      &logit("Doing BASIC test on c$num_length $num_id\n");
      &factor_basic($num_id, $n, $notes);
    } elsif ("$method" eq "ECM") {
      &logit("Doing ECM on c$num_length $num_id: ($notes)\n");
      &factor_ecm($num_id, $n, $notes);
    } elsif ("$method" eq "MPQS") {
      &logit("Doing MPQS on c$num_length $num_id\n");
      &factor_mpqs($num_id, $n, $notes);
    } else {
      &logit("Unknown method $method for c$num_length $num_id = $n\n");
    }
  }
}
...
sub return_work() {
  ...
  &logit($Factors{basic} . " " . $Factors{ecm} . " " . $Factors{mpqs} . "\n");
